...
from constants import bug_report_channel, submittal_confirmation_channel
...
class AppCommands(commands.Cog):
    ...
    @discord.app_commands.describe(message_id='The ID of the bug report message to resolve')
    @discord.app_commands.rename(message_id='message-id')
    async def resolve_bug(self, interaction: discord.Interaction, message_id: str):
        int_message_id = int(message_id)
        message: discord.Message = await self.bot.get_channel(bug_report_channel).fetch_message(int_message_id)
