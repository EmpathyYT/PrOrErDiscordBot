...
class BugVoteController(VoteController):
    ...
    async def add_user_vote(self, message_id, user_id):
        await self.db_provider.add_bug_user(message_id, user_id)

    async def remove_user_vote(self, message_id, user_id):
        await self.db_provider.remove_bug_user(message_id, user_id)

    async def get_user_votes(self, message_id):
        return await self.db_provider.get_bug_users(message_id)
